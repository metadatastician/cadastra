...
# CADASTRA ABI/FFI Documentation
...
│       └── include/
│           └── cadastra.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── cadastra.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/cadastra.h
```
...
int main() {
    void* handle = cadastra_init();
    if (!handle) return 1;

    int result = cadastra_process(handle, 42);
    if (result != 0) {
        const char* err = cadastra_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    cadastra_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lcadastra -L./zig-out/lib
```

### From Idris2

```idris
import CADASTRA.ABI.Foreign
...
#[link(name = "cadastra")]
extern "C" {
    fn cadastra_init() -> *mut std::ffi::c_void;
    fn cadastra_free(handle: *mut std::ffi::c_void);
    fn cadastra_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = cadastra_init();
        assert!(!handle.is_null());

        let result = cadastra_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcadastra = "libcadastra"

function init()
    handle = ccall((:cadastra_init, libcadastra), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:cadastra_process, libcadastra), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:cadastra_free, libcadastra), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/cadastra.h
   ```
